...
<?php
    ...
    else
    {
        ...
        $sql ="INSERT INTO patient(patientname,admissiondate,admissiontime,address,mobileno,city,pincode,loginid,password,emergency,contact,bloodgroup,gender,dob,status) values('$_POST[patientname]','$_POST[admissiondate]','$_POST[admissiontime]','$_POST[address]','$_POST[mobilenumber]','$_POST[city]','$_POST[pincode]','$_POST[loginid]','$pass','$_POST[emergency]','$_POST[contact]','$_POST[select2]','$_POST[gender]','$_POST[dateofbirth]','Active')";
        if($qsql = mysqli_query($conn,$sql))
        {
?>
            <div class="popup popup--icon -success js_success-popup popup--visible">
...
<form id="main" method="post" action="" enctype="multipart/form-data">
    ...
    <div class="form-group row">
        ...
        <select class="form-control show-tick" name="select2" id="select2">
                ...
                <?php
            $arr = array("Faculty","Student","University Staff");
            foreach($arr as $val)
            {
                if($val == $rsedit['patienttype'])
                {
        ...
        <select class="form-control show-tick" name="select2" id="select2">
                <option value="">Select</option>
                <?php
            $arr = array("Accountancy","Entrepreneurship","Electronics Engineering","Industrial Engineering","Information Technology","QCU");
            foreach($arr as $val)
            {
                if($val == $rsedit['department'])
                {
                    echo "<option value='$val' selected>$val</option>";
                }
                else
    ...
    <div class="form-group row">
        ...
        <div class="col-sm-4">
             <input class="form-control" type="text" name="emergency" id="emergency"
                        value="<?php if(isset($_GET['editid'])) { echo $rsedit['emergency']; } ?>" />
            <span class="messages"></span>
        </div>
